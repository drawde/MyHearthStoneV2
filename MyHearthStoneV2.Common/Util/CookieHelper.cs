...
namespace MyHearthStoneV2.Common.Util
{
    public class CookieHelper
    {
        ...
        public static string GetCookieValue(string cookieName, string key)
        {
            ...
            if (request != null)
                return HttpUtility.UrlDecode(GetCookieValue(request.Cookies[cookieName], key), Encoding.UTF8);
            return "";
        }
        /// <summary>
        /// 获得Cookie的子键值
        ...
        public static string GetCookieValue(HttpCookie cookie, string key)
        {
            if (cookie != null)
            {
                if (!string.IsNullOrEmpty(key) && cookie.HasKeys)
                    return HttpUtility.UrlDecode(cookie.Values[key], Encoding.UTF8);
                else
        ...
        {
            value = HttpUtility.UrlEncode(value, Encoding.GetEncoding("UTF=8"));
            HttpCookie cookie = new HttpCookie(cookieName);
            cookie.Expires = expires;
            cookie.Values.Add(key, value);
            AddCookie(cookie);
