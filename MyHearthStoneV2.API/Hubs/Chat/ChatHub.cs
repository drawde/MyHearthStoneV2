...
namespace MyHearthStoneV2.API.Hubs.Chat
{
    ...
    {
        private IList<ChatUser> userList = ChatUserCache.userList;
        ...
        {
            //var userInfo = userList.Where(x => x.UserCode.Equals(id) && x.Name.Equals(name)).FirstOrDefault();
            //userInfo.count = 0;  //收到心跳，重置计数器
        }

        public void SendLogin(string id, string name)
        {
            //if (!userList.Any(c => c.UserCode == id))
            //{
            //    var userInfo = new ChatUser() { UserCode = id, Name = name };
            //    userInfo.action += () =>
            //    {
